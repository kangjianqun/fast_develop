...
typedef ScrollableNotifier = void Function(double offset);
...
class ScrollablePositionedList extends StatefulWidget {
  ...
  final ScrollableNotifier? scrollableNotifier;
...
class ItemScrollController {
  ...
  late _ScrollablePositionedListState? _scrollableListState;
  ...
  void _detach() {
    if (_scrollableListState != null) _scrollableListState = null;
  }
}
...
class _ScrollablePositionedListState extends State<ScrollablePositionedList>
  ...
  late Function? cancelScrollCallback;
  late Function? endScrollCallback;
  ...
  void initState() {
    ...
    frontItemPositionNotifier.itemPositions.addListener(_updateOffset);
    backItemPositionNotifier.itemPositions.addListener(_updateOffset);
  }
  ...
  void dispose() {
    ...
    frontItemPositionNotifier.itemPositions.removeListener(_updateOffset);
    backItemPositionNotifier.itemPositions.removeListener(_updateOffset);
    super.dispose();
  }
  ...
  Future<void> _startScroll(
    ...
    final ItemPosition? itemPosition = (_showFrontList
        ...
        .firstWhere((ItemPosition itemPosition) => itemPosition.index == index,
            orElse: () => ItemPosition(
                index: -1, itemLeadingEdge: 0, itemTrailingEdge: 0));
    if (itemPosition != null && itemPosition.index > -1) {
  ...
  void _updateOffset() {
    ScrollController _scroll =
        _showFrontList ? frontScrollController : backScrollController;
    double offset = 0.0;
    if (_scroll.position.minScrollExtent < 0) {
      offset = _scroll.position.minScrollExtent - _scroll.offset;
    } else {
      offset = _scroll.offset;
    }
    widget.scrollableNotifier(offset.abs());
  }
